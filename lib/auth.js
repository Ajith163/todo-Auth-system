    ...
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        try {
          ...
          // Check if user is approved - NO AUTO-APPROVAL
          if (!user[0].approved) {
            console.log('❌ User not approved:', user[0].email)
            throw new Error('Account not approved yet. Please contact an administrator.');
          }

          // Check if user is rejected
          if (user[0].rejected) {
            console.log('❌ User rejected:', user[0].email)
            throw new Error('Account has been rejected. Please contact an administrator.');
          }
